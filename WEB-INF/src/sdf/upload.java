...
//   截取formType    
   ...
   //   截取filename
		...
		String  uploadsql = "update userinfo set icon='"+icon+"' where email='"+email+"'";
		asql.executeUpdate(uploadsql);
    	RequestDispatcher go=request.getRequestDispatcher("iframe_person_alter.jsp");
    	go.forward(request, response);
    }else{
    	request.setAttribute("s", "请上传格式为jpg的图片!");
    	request.setAttribute("email", email);
    	RequestDispatcher go=request.getRequestDispatcher("iframe_person_alter.jsp");
    	go.forward(request, response);
    }
