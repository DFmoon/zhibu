...
public class thingpic_upload extends HttpServlet {

public thingpic_upload(){
   super();
}
...
//   截取formType    
   ...
   //   截取filename
		...
		String  sql = "update changething set change_pic='"+icon+"' where thingid='"+thingid+"'";
		asql.executeUpdate(sql);
		request.setAttribute("pic", "上传成功,点击立即交换就可以继续订购啦！");
    	...
    	request.setAttribute("pic", "请上传格式为jpg的图片!");
    	request.setAttribute("houseid",houseid);
		request.setAttribute("thingid",thingid);
