...
<script>
/*返回修改*/ 
function back(houseid)
{
	document.location="house.jsp?houseid="+houseid;
}  
/*数量+1*/  
...
<%
	...
	String sql="select * from houseinfo1 where houseid='"+houseid+"'";
...
<form id="order_detail" method="post" action="/add_order1?houseid=<%=houseid%>">
    ...
    <label>单价：<input name="univalent" value="<%=univalent%>" style="width:60px;" readonly="readonly" />元/晚</label><br />
    <label>数量：
    ...
    <input type="button" class="btn" style="margin-left:80px; width:80px;" value="返回修改" onclick="back(<%=houseid%>)" />
</form>
